...
mpg_make_model <- mpg %>% # Start with mpg
  unite(col = make_model, # Create a new column called 'make_model'
        ...
        sep = "CARS!" # Puts "HEY!" between the pieces (default is underscore)
        ) 
...
mpg_model_class <- mpg %>% #start with mpg dataset
  unite(col = model_class, #create new column called "model_class"
        ...
        remove = FALSE) #keep original model and class columns
...
```{r}
# Your code here: 
some_dates <- c(11061972, 02142007, 01301843) #vector of dates without spaces
...
gasol_hookshots <- lakers %>% 
  filter(player == "Pau Gasol", type == "hook") %>% #only include Pau Gasol (player) and hook (type)
  mutate(date_new = ymd(date)) %>% #add date column with "Date" class
  mutate(month = month(date_new)) #add month column

hook_by_month <- gasol_hookshots %>% 
  count(month) #number of hook shots recorded for each month
```
